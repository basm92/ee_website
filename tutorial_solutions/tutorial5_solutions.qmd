...
1.  Using the `wagepan` panel data, estimate a **Random Effects** model where `lwage` is a function of the time-varying predictors `married` and `union`, and the (mostly) time-invariant predictor `educ`.^[In Python, the random effects model can be estimated using the `linearmodels` library.]
...
beta_fe, beta_re = fe_model.params, re_model.params
cov_fe, cov_re = fe_model.cov, re_model.cov
...
hausman_stat = diff.T @ abs(np.linalg.inv(cov_diff)) @ diff

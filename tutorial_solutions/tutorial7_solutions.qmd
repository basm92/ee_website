...
2. Plot the average log wages for the high-construction and low-construction regions for the pre-treatment cohorts (-4 until -1). Does this visual evidence support the parallel trends assumption for a simple DiD analysis around 1989? Discuss what you see in the cohorts leading up to the treatment.
...
The parallel trends test asks: "Between cohorts of children who were all too old to be affected by the program, was the trend in educational attainment similar between high-construction and low-construction regions?" You would formally test for pre-trends by estimating a regression of the following form:$Y_{ic} = \alpha + \sum_{k=-4}^{-1} \beta_k D_{k} \times Treated_c + \beta Treated_c + \sum_{k=-1}^{-1} \gamma_k D_k + u_{ic}$

Where: $c$ is the cohort, $i$ is the individual, $D_k$ is a dummy variable equal to 1 if the individual belongs to cohort $k$, and $Treated_c$ is a dummy variable equal to 1 if the individual belongs to a high-construction region. You would be looking at the coefficients $\beta_k$ for $k = -4, -3, -2, -1$. If the parallel trends assumption holds, you would hope to find that these coefficients are statistically indistinguishable from zero, indicating that there were no differential trends in educational attainment between high-construction and low-construction regions prior to the treatment period. You could conduct an F-test to jointly test the null hypothesis that all $\beta_k = 0$ for $k = -4, -3, -2, -1$.
...
2. Plot the average log wages for the high-construction and low-construction regions for the pre-treatment cohorts. Does this visual evidence support the parallel trends assumption for a simple DiD analysis around 1989? Discuss what you see in the cohorts leading up to the treatment.

3. Briefly outline how you would formally test for pre-trends using a regression framework, as described in the lecture. What coefficients would you be looking at, and what would you hope to find?
...
The observation that the yields of the "treatment" farms were already declining in the years leading up to the adoption, while the yields of the "control" farms were stable, **violates** the parallel trends assumption. The pre-treatment trends are not parallel.

**(b) Direction of Bias**

Assuming the researcher found a positive DiD estimate, this estimate is likely an **overestimate** of the true causal effect.

**Reasoning:**

The parallel trends assumption allows us to use the trend of the control group to construct the counterfactual trend for the treatment group. In this case, the control group's trend is stable. Therefore, the DiD calculation would assume that, in the absence of the fertilizer, the treatment group's yields would have also remained stable.

However, the pre-treatment data suggests that the treatment group's yields were already on a downward trajectory. Therefore, a more plausible counterfactual is that their yields would have *continued to decline* had they not adopted the new fertilizer. Because the DiD estimate is calculated relative to a counterfactual of stable yields (a higher baseline), the observed post-treatment yields of the treatment group will appear artificially high, leading to an overestimation of the fertilizer's true positive effect.
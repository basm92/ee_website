...
mean ln_wage, over(treated post)
...
Where: $c$ is the cohort, $i$ is the individual, $D_k$ is a dummy variable equal to 1 if the individual belongs to cohort $k$, and $Treated_i$ is a dummy variable equal to 1 if the individual comes from a high-construction region. You would be looking at the coefficients $\beta_k$ for $k = -4, -3, -2, -1$. If the parallel trends assumption holds, you would hope to find that these coefficients are statistically indistinguishable from zero, indicating that there were no differential trends in educational attainment between high-construction and low-construction regions prior to the treatment period. You could also conduct an F-test to jointly test the null hypothesis that all $\beta_k = 0$ for $k = -4, -3, -2, -1$.
...
regress yeduc ib-1.cohort##i.treated

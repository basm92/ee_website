...
subtitle: "Lecture 7: Instrumental Variables"
mainfont: Lato
...
format:
  revealjs: 
    ...
    logo: logo.svg
    css: styles.css
    footer: 'Empirical Economics: Lecture 7 - Instrumental Variables'
    include-in-header:
      - file: preamble.tex
resources:
  - demo.pdf
...
- For Ordinary Least Squares (OLS) to be unbiased, we need the regressor $X$ to be uncorrelated with the error term $\epsilon$: $E[X_i \epsilon_i] = 0 \quad \text{or} \quad Cov(X_i, \epsilon_i) = 0$
...
- **The Endogeneity Problem (OVB):** $U$ is an unobserved confounder.

...
- By definition of the conditional expectation, $E(u_i|x) = 0$. The average of the unobserved factors does not depend on the level of education.
...
- **The sample covariance between the regressor ($x$) and the OLS residuals ($u$) is zero:**
...
Under assumptions **SLR.1 through SLR.5** (all four SLR assumptions plus _homoskedasticity_), the variance of the OLS slope estimator is:
...
- We plot the distribution of the 1,000 estimated coefficients is centered around the true value of 5 (the solid red line).
...
- Now we have all the pieces:
  ...
  - Plug in our estimate $\hat{\sigma}^2$ for the unknown $\sigma^2$

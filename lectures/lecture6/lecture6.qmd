...
- **Crucial Point - The Exclusion Restriction:** 
  - For robust identification, there must be at least one variable in the selection equation (Z) that is **not** in the outcome equation (X). 
...
# --- 1. Data Generation with Selection Bias ---

# Set parameters
np.random.seed(42)
n_obs = 2000
beta = np.array([5, 2])    # True coefficients for the outcome equation [intercept, slope]
gamma = np.array([0.5, 1, 1.5]) # True coefficients for the selection equation
rho = 0.7                  # Correlation between errors of outcome and selection equations

# Generate independent variables
# The "instrument" z1 affects selection but not the outcome y
x0 = np.ones(n_obs)
x1 = np.random.uniform(-5, 5, n_obs)
z1 = np.random.normal(0, 1, n_obs) # This is our instrumental variable

# Create the design matrices
# Outcome equation depends on intercept and x1
X = np.column_stack((x0, x1))
# Selection equation depends on intercept, x1, and the instrument z1
Z = np.column_stack((x0, x1, z1))

# Generate correlated errors for the two equations
cov_matrix = np.array([[1, rho], [rho, 1]])
errors = np.random.multivariate_normal([0, 0], cov_matrix, size=n_obs)
error_outcome = errors[:, 0]  # Error for the outcome equation (v)
error_selection = errors[:, 1] # Error for the selection equation (u)

# Generate the latent (unobserved) outcome variable
y_latent = X @ beta + error_outcome

# Generate the latent selection variable and the observed selection decision
selection_latent = Z @ gamma + error_selection
selection = selection_latent > 0  # We only observe y if selection_latent is positive

# Create the observed y, which has missing values (NaN) due to selection
y_observed = y_latent.copy()
y_observed[~selection] = np.nan # Set y to NaN for unselected observations
...
# a) OLS Model (on selected data only)
df_selected = df.dropna()
X_selected = sm.add_constant(df_selected['x1'])

ols_model = sm.OLS(df_selected['y_observed'], X_selected)
ols_results = ols_model.fit()

# b) Heckman Selection Model
# The model needs the full dataset (with NaNs in y)
# Outcome equation: y_observed ~ 1 + x1
# Selection equation: selection ~ 1 + x1 + z1
outcome_exog = sm.add_constant(df['x1'])
selection_exog = sm.add_constant(df[['x1', 'z1']])

heckman_model = Heckman(df['y_observed'], outcome_exog, selection_exog=selection_exog)
heckman_results = heckman_model.fit()
...
# Generate a smooth range of x values for plotting the lines
...
# 3. Plot the Heckman line
# The coefficients are stored in heckman_results.beta
y_heckman = X_plot @ heckman_results.beta
ax.plot(x_plot, y_heckman, 'g-.', lw=3, label=f'Heckman Fit (Corrected Slope={heckman_results.beta[1]:.2f})')

# Final plot adjustments
ax.set_title('OLS vs. Heckman Model for Selection Bias', fontsize=16)

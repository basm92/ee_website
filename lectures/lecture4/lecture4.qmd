...
:::{.callout-tip title="Example: Finite Expectation Variance of $X$ when $X \sim AR(1)$"}
...
# Optional: Visualize the data and the correlated errors
# plot(u, type = 'l', main = "Simulated Serially Correlated Errors")
# plot(Y ~ X, data = sim_data, main = "Simulated Regression Data")
...
- Sometimes, the effect of X on Y might persist indefinitely, requiring an infinite number of lags. This is not practical to estimate.
- The Koyck model assumes that the coefficients decline geometrically: $\beta_s = \beta_0 \lambda^s$, where $0 < \lambda < 1$.
...
- This is a form of an ARDL model.. 
...
- **Key Takeaway**: The ARDL model powerfully separates the **immediate impacts** ($\beta_j$) from the **total equilibrium effect** ($\theta$), which accounts for the feedback from Y's own past values.

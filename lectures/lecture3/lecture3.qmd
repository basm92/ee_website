...
- Detecting spurious regression: **Breusch-Godfrey** or **Dickey-Fuller** tests
  - Ex ante: if both series $Y_t$ and $X_t$ have a unit root (i.e., you fail to reject $H_0$ for both), then running a simple OLS regression of Y on X is at high risk of being spurious.
  - Ex-post: the "error" in this model is not a random, white-noise process. Instead, the residuals themselves are highly persistent and non-stationary. Detectable via Breusch-Godfrey.

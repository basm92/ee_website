...
- To evaluate the variance of the sum, we must make assumptions about the error terms, $u_i$. The **Linear Model** imposes two key assumptions on the errors:
...
- Let's re-derive the variance of $\hat{\beta}_1$ under the clustering assumption. We start again from:
  ...
  $$
    Var(\hat{\beta}_1) = \frac{1}{\left(\sum_{g,i} (x_{gi} - \bar{x})^2\right)^2} Var\left(\sum_{g=1}^G \sum_{i=1}^{N_g} (x_{gi} - \bar{x})u_{gi}\right)
  $$
...
- Since errors are uncorrelated *across* clusters, the variance of this sum is the sum of the variances:
  ...
  $$
    = \sum_{g=1}^G Var\left( \sum_{i=1}^{N_g} (x_{gi} - \bar{x})u_{gi} \right)
  $$
...
- Plugging this back into our main variance formula for $\hat{\beta}_1$, we get the **cluster-robust variance estimator**:
  ...
  $$
    V\hat{a}r_C(\hat{\beta}_1) = \frac{1}{\left(\sum_{g,i} (x_{gi} - \bar{x})^2\right)^2} \left[ \sum_{g=1}^G \left( \sum_{i=1}^{N_g} (x_{gi} - \bar{x})\hat{u}_{gi} \right)^2 \right]
  $$
...
- Let's compare this to the **heteroskedasticity-robust estimator**:
  ...
  $$
    V\hat{a}r_W(\hat{\beta}_1) = \frac{1}{\left(\sum_i (x_i - \bar{x})^2\right)^2} \left[ \sum_{i=1}^N (x_i - \bar{x})^2 \hat{u}_i^2 \right]
  $$

...
a)  Simulate a population A large population of 100,000 data points is drawn from an Exponential distribution with a mean ($\mu$) of 10. A histogram of this population data would be highly right-skewed, with most values clustered near zero and a long tail extending to the right.
...
b)  Draw 5,000 different random samples From this skewed population, we draw 5,000 separate random samples, each containing `n=50` observations.
...
c)  For each sample, calculate its sample mean We compute the mean for each of the 5,000 samples, resulting in a list of 5,000 sample means.
...
d)  Plot a histogram of the 5,000 sample means (While a plot cannot be displayed here, we can describe the result).
